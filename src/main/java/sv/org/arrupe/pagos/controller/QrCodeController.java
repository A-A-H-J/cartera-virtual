...
public class QrCodeController {
    ...
    /**
     * Endpoint para que un VENDEDOR solicite un pago a un CLIENTE. Reemplaza el
     * antiguo "process-payment".
     */
    @PostMapping("/request-payment")
    public ResponseEntity<?> requestPayment(@RequestBody PaymentRequest paymentRequest) {
        try {
            Transaccion transaccion = transaccionService.solicitarPago(
            ...
            return ResponseEntity.ok(Map.of(
                    "message", "Solicitud de pago enviada con éxito.",
                    "transactionId", transaccion.getIdTransaccion() // <-- agregamos el ID
            ));
        } catch (Exception e) {
